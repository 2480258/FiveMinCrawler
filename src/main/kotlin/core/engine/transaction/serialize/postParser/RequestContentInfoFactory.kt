...
import arrow.core.*
import core.engine.*
import core.request.RequestHeaderProfile
import java.net.URI
...
class ExtAttrRequestFactory(private val attributeTargetName : String, private val selector: LinkSelector) : RequestFactory{
    ...
    private fun create(info : Iterable<LinkExtractedInfo>, token : RequestToken, parentURI: URI) : RequestLinkInfo{
        return RequestLinkInfo(attributeTargetName, info.map{HttpRequestImpl(
            Some(token),
            it.absoluteURI,
